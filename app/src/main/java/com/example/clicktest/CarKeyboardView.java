        ...
        if (keys != null && keys.size() > 0) {
            ...
            for (Keyboard.Key key : keys) {
                if (key.codes[0] == -4) {
                    ...
                    dr.draw(canvas);
                    if (key.label != null) {
                        paint.setTextSize(ScreenUtil.sp2px(getContext(),18));
                        //设置字体颜色
...
//                        paint.setShadowLayer(0, 0, 0, 0);
                    ...
                    Drawable dr = getContext().getResources().getDrawable(R.drawable.ad_count_down_bg);
                    dr.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
                    dr.draw(canvas);
